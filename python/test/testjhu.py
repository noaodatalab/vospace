...
BASE_URI = 'http://dimm.pha.jhu.edu:8080/vospace-2.0/rest/'
ROOT_NODE = 'vos://edu.jhu!vospace'
...
def clearAll():
  h = get_h()
  resp, content = h.request(BASE_URI + 'nodes', 'DELETE', headers = get_header())
  print resp, content
...
def test_start_uws(h, resource, body):
  ...
  """
  # Submit job
  resp, content = h.request(BASE_URI + resource, 'POST', body = body, headers = get_header())
  ...
  # Run job
  data = dict(PHASE = "RUN")
  resp, content = h.request(BASE_URI + '%s/%s/phase' % (resource, job.jobId), 'POST', urlencode(data), headers = get_header())
  assert int(resp.previous['status']) == 303
  assert int(resp['status']) == 200
  job = Job(content)
...
def test_uws(h, resource, body, fail = False, summary = ''):
  ...
  while content not in ['COMPLETED', 'ERROR']:
    resp, content = h.request(BASE_URI + '%s/%s/phase' % (resource, jobid), headers = get_header())
    assert int(resp['status']) == 200
    sleep(1)
  resp, content = h.request(BASE_URI + '%s/%s' % (resource, jobid), headers = get_header())
  assert int(resp['status']) == 200
  job = Job(content)
#  print content
  if fail:
...
class GetProtocolsTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    """
    h = get_h()
    self.resp, self.content = h.request(BASE_URI + 'protocols', headers = get_header())
...
class GetViewsTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    """
    h = get_h()
    self.resp, self.content = h.request(BASE_URI + 'views', headers = get_header())
...
class GetPropertiesTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    """
    h = get_h()
    self.resp, self.content = h.request(BASE_URI + 'properties', headers = get_header())
...
class CreateNodeTestCase(unittest.TestCase):
  ...
  def test_create_node(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node1', 'PUT', body = node.tostring(), headers = get_header())
  ...
  def test_create_datanode(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node2', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_create_linknode(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node3', 'PUT', body = etree.tostring(node), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_create_unstructureddatanode(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node4', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_create_structureddatanode(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node5', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_create_containernode(self):
    ...
    print node.tostring()
    resp, content = self.h.request(BASE_URI + 'nodes/node6', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_create_unsupported_node(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node7', 'PUT', body = etree.tostring(node), headers = get_header())
    self.assertEqual(int(resp['status']), 400)
  ...
  def test_create_node_with_readonly_property(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node8', 'PUT', body = etree.tostring(node), headers = get_header())
...
class MoveNodeTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.h = get_h()
    resp, content = self.h.request(BASE_URI + 'nodes/node10', 'PUT', body = node.tostring(), headers = get_header())
    self.xm = XMLMatcher()
  ...
  def test_move_basic_node(self):
    ...
    # Check move
    resp, content = self.h.request(BASE_URI + 'nodes/node10', headers = get_header())
    self.assertEqual(int(resp['status']), 404)
    resp, content = self.h.request(BASE_URI + 'nodes/nodem1', headers = get_header())
  ...
  def test_move_basic_node_into_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node10', headers = get_header())
    self.assertEqual(int(resp['status']), 404)
    resp, content = self.h.request(BASE_URI + 'nodes/node6', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    nf = NodeFactory()
    container = nf.get_node(content)
    assert_contains(container.nodes, ROOT_NODE + "/node6/node10")
    resp, content = self.h.request(BASE_URI + 'nodes', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    container = nf.get_node(content)
  ...
  def test_move_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node6', headers = get_header())
    self.assertEqual(int(resp['status']), 404)
    resp, content = self.h.request(BASE_URI + 'nodes/nodem6', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    nf = NodeFactory()
    container = nf.get_node(content)
  ...
  def test_move_container_into_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node11', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
    # Move container
    transfer = etree.parse('test/transfer.xml')
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/nodem6', headers = get_header())
    self.assertEqual(int(resp['status']), 404)
    resp, content = self.h.request(BASE_URI + 'nodes/node11/nodem6', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    nf = NodeFactory()
    container = nf.get_node(content)
...
class CopyNodeTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.h = get_h()
    resp, content = self.h.request(BASE_URI + 'nodes/node12', 'PUT', body = node.tostring(), headers = get_header())
    self.xm = XMLMatcher()
    self.nf = NodeFactory()
#    self.assertEqual(int(resp['status']), 201)
...
#    self.assertEqual(int(resp['status']), 200)
  ...
  def test_copy_basic_node(self):
    ...
    test_uws(self.h, 'transfers', etree.tostring(transfer))
    # Check copy
    resp, content = self.h.request(BASE_URI + 'nodes/node12', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    resp, content = self.h.request(BASE_URI + 'nodes/nodec1', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_copy_basic_node_into_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node12', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    resp, content = self.h.request(BASE_URI + 'nodes/node11/node12', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    resp, content = self.h.request(BASE_URI + 'nodes/node11', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_copy_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node11', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    resp, content = self.h.request(BASE_URI + 'nodes/nodec11', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    container = self.nf.get_node(content)
    assert_contains(container.nodes, ROOT_NODE + "/nodec11/node12")
    assert_contains(container.nodes, ROOT_NODE + "/nodec11/nodem6")
    resp, content = self.h.request(BASE_URI + 'nodes/nodec11/nodem6', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    container = self.nf.get_node(content)
  ...
  def test_copy_container_into_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node13', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
    # Move container
    transfer = etree.parse('test/transfer.xml')
    set_transfer_target(transfer, ROOT_NODE + '/node11')
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node11', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    resp, content = self.h.request(BASE_URI + 'nodes/node13/node11', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    # Check container listings
    container = self.nf.get_node(content)
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node13/node11/nodem6', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    container = self.nf.get_node(content)
...
class DeleteNodeTestCase(unittest.TestCase):
  ...
  def test_delete_node(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node9', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
    resp, content = self.h.request(BASE_URI + 'nodes/node9', 'DELETE', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_delete_unknown_node(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/nodexx', 'DELETE', headers = get_header())
    self.assertEqual(int(resp['status']), 404)
  ...
  def test_delete_container(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/nodec1', 'DELETE', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
...
class GetNodeTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'PUT', body = node, headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def tearDown(self):
    ...
    Tidy up after test
    """
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'DELETE', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_get_node(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/node16', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_get_min_detail(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/node16?detail=min', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_get_max_detail(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/node16?detail=max', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    self.xm.assert_xml_strings(content, open('test/fullnode.xml').read())
  ...
  def test_get_properties(self):
    ...
    """
    resp, content = self.h.request(BASE_URI + 'nodes/node16?detail=properties', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_get_protocols(self):
    ...
    """
    protocol = Protocol()
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'POST', body = protocol.tostring(), headers = get_header())
  ...
  def test_get_container(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon', 'PUT', body = node.tostring(), headers = get_header())
    node = Node()
    node.uri = ROOT_NODE + '/nodecon/nodea'
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon/nodea', 'PUT', body = node.tostring(), headers = get_header())
    node.uri = ROOT_NODE + '/nodecon/nodeb'
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon/nodeb', 'PUT', body = node.tostring(), headers = get_header())
    node.uri = ROOT_NODE + '/nodecon/nodec'
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon/nodec', 'PUT', body = node.tostring(), headers = get_header())
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    container = self.nf.get_node(content)
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon', 'DELETE', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_get_container_with_offset(self):
    ...
    node.uri = ROOT_NODE + '/nodecon/nodec'
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon/nodec', 'PUT', body = node.tostring(), headers = get_header())
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon?uri=vos://nvo.caltech!vospace/nodecon/nodeb&offset=1', headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    container = self.nf.get_node(content)
    assert_contains(container.nodes, ROOT_NODE + "/nodecon/nodeb")
    resp, content = self.h.request(BASE_URI + 'nodes/nodecon', "DELETE", headers = get_header())
...
class SetNodeTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'PUT', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 201)
  ...
  def test_set_node(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'POST', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 200)
  ...
  def test_delete_property(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node16', 'POST', body = etree.tostring(nodexml), headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    newnode = self.nf.get_node(content)
  ...
  def test_set_children(self):
    ...
    resp, content = self.h.request(BASE_URI + 'nodes/node11', 'POST', body = node.tostring(), headers = get_header())
    self.assertEqual(int(resp['status']), 200)
    newnode = self.nf.get_node(content)
...
class PushToVoSpaceTestCase(unittest.TestCase):
  ...
  def handle_job(self, transfer, fail = False, summary = None):
    ...
    content = 'QUEUED'
    while content not in ['COMPLETED', 'ERROR']:
      resp, content = self.h.request(BASE_URI + 'transfers/%s/phase' % jobid, headers = get_header())
      self.assertEqual(int(resp['status']), 200)
      # Transfer data
      if content == 'EXECUTING':
        resp, content = self.h.request(BASE_URI + 'transfers/%s' % jobid, headers = get_header())
        job = Job(content)
        ...
        for protocol in transfer.protocols:
          if protocol.uri == 'ivo://ivoa.net/vospace/core#httpput':
            ...
            resp, content = self.h.request(protocol.endpoint, 'PUT', body = file, headers = get_header())
        break
      sleep(1)
  ...
  def test_quick_push_to_vospace(self):
    ...
    set_transfer_protocol(self.transfer, 'ivo://ivoa.net/vospace/core#httpput')
    # Submit job
    resp, content = self.h.request(BASE_URI + "sync", 'POST', body = etree.tostring(self.transfer), headers = get_header())
    assert int(resp.previous['status']) == 303
    assert int(resp['status']) == 200
  ...
  def test_push_to_vospace_with_unsupported_view(self):
    ...
    set_transfer_protocol(self.transfer, 'ivo://ivoa.net/vospace/core#httpput')
    resp, content = self.h.request(BASE_URI + 'transfers', 'POST', body = etree.tostring(self.transfer), headers = get_header())
    self.assertEqual(int(resp['status']), 500)
...
class PullFromVoSpaceTestCase(unittest.TestCase):
  ...
  def handle_job(self, transfer, fail = False, summary = None):
    ...
    while content not in ['COMPLETED', 'ERROR']:
      ...
      if content == 'EXECUTING':
        ...
        assert len(job.results) > 0
        results = job.results['transferDetails']
        resp, content = self.h.request(results, headers = get_header())
        transfer = Transfer(content)
        for protocol in transfer.protocols:
          if protocol.uri == 'ivo://ivoa.net/vospace/core#httpget':
            resp, content = self.h.request(protocol.endpoint, headers = get_header())
  ...
  def test_quick_pull_from_vospace(self):
    ...
    Test transferring data from the space: node12
    """
    resp, content = self.h.request(BASE_URI + "nodes/node12?view=data", headers = get_header())
    file = open('test/check.vot', 'wb')
    file.write(content)
    file.close()
    # Verify files
  ...
  def test_quick_pull_from_vospace_with_unknown_node(self):
    ...
    Test transferring data from the space: node12
    """
    resp, content = self.h.request(BASE_URI + "nodes/node123?view=data", headers = get_header())
    self.assertEqual(int(resp['status']), 404)
