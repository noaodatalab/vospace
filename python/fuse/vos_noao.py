...
#SERVER = os.getenv('VOSPACE_WEBSERVICE', 'localhost:8080/vospace-2.0')
CADC_GMS_PREFIX = "ivo://datalab.noao.edu/gms#"
...
class Node:
    ...
    def update(self):
        """Update the convenience links of this node as we update the xml file"""
    ...
    def __str__(self):
        ...
        ElementTree.ElementTree(self.node).write(file) # MJG
        return "".join(data)
    ...
    def setattr(self, attr={}):
        ...
        if not sdate:
            ...
            mtime = time.mktime(time.strptime(sdate[0:19], # MJG
                                              '%Y-%m-%dT%H:%M:%S'))
        ...
        ## Set the GROUP permissions # MJG MAYBE HERE
    ...
    # Check group value of parent - does this work with CADC_GMS_PREFIX added?
    # What values are in group? MJG
    ...
    def changeProp(self, key, value):
        ...
        logger.debug("Property change %s %s" % (key, value)) # MJG
        ...
        # Workaround for UWS library issues MJG 
            ...
            if method == "PUT":
                ...
                if ext in ['.fz', '.fits', 'fit']:
                    ...
                    if contentType is None: contentType = "text/xml" # MJG 
...
class Client:
    ...
    def transfer(self, uri, direction, view="defaultview", cutout=None):
        ...
        views = {"defaultview": "%s#%s" % (Node.IVOAURL, "defaultview"),
                 "data": "ivo://datalab.noao.edu/vospace/view#data",
                 "cutout": "ivo://datalab.noao.edu/vospace/view#cutout"
        }
