...
CONNECTION_TIMEOUT = 30  # seconds before HTTP connection should drop, should be less that DAEMON timeout in myvofs
...
VOSPACE_GMS_PREFIX = "ivo://nvo.caltech/gms#"
VOSPACE_CERTFILE = ""
...
class Connection:
    ...
    def get_connection(self, url):
        ...
        parts = URLparse(url)
        connection_count_logger.debug("Opening connection {0} to {1}://{2} using {3}".format(CONNECTION_COUNTER, parts.scheme, parts.netloc, self.security_token))
...
#        ElementTree.ElementTree(self.node).write(file, encoding="UTF-8")
    ...
    def setattr(self, attr={}):
        ...
        # MJG: check groupwrite/groupread
...
#           MJG 12/10/14 - closing the connection here wipes the response
#            buffer - is this an issue with the version of httplib being used?
#            self.httpCon.close()
            logger.debug("closed.")
        return self.closed
...
#                 'datalab.noao.edu~vospace': SERVER}
    ...
    def __init__(self, rootNode=None, conn=None,
                 http_debug=True, security_token = None):
        ...
        if not isinstance(conn, Connection):
            conn = Connection(security_token = security_token, http_debug = http_debug)
...
#            do_shortcut = self.cadc_short_cut
    ...
    def transfer(self, uri, direction, view="defaultview", cutout=None):
        ...
        views = {"defaultview": "%s#%s" % (Node.IVOAURL, "defaultview"),
                 "data": "ivo://datalab.noao.edu/vospace/view#data", # NOT SUPPORTED YET  (MJG)
                 "cutout": "ivo://datalab.noao.edu/vospace/view#cutout" # NOT SUPPORTED YET (MJG)
        }
